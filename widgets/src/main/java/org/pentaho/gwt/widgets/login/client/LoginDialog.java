...
public class LoginDialog extends PromptDialogBox {
  ...
  private final IDialogCallback myCallback = new IDialogCallback() {
    ...
    public void okPressed() {
      ...
      String requestData =
          "j_username=" + URL.encodeComponent( username ) + "&j_password=" + URL.encodeComponent( password ); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public LoginDialog() {
    ...
    // TODO Once we have a service available which actually returns the show user list information we can uncomment this
    // section
    /*
     * RequestBuilder requestBuilder = new RequestBuilder(RequestBuilder.GET, getWebAppPath() +
     * "content/ws-run/loginService/isShowUsersList"); //$NON-NLS-1$ try { requestBuilder.sendRequest(null, new
     * RequestCallback() { public void onError(Request request, Throwable exception) { getLoginSettingsAndShow(false); }
     * 
  ...
  public void getLoginSettingsAndShow( final boolean showUsersListDefault ) {
     ...
     * String[] { userIdList.get(i).trim(), passwordList.get(i).trim() }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ }
     * // provide the opportunity to override showUsersList with a setting if (showUser != null) { //$NON-NLS-1$
     * showUsersList = showUser; //$NON-NLS-1$ //$NON-NLS-2$ } // get the default 'open in new window' flag, this is the
     * default, overridden by a cookie if(openInNewWindow != null) { openInNewWindowDefault = openInNewWindow;
     * //$NON-NLS-1$ //$NON-NLS-2$ } } setContent(buildLoginPanel(openInNewWindowDefault)); if (isAttached() &&
     * isVisible()) { center(); } } }); } catch (RequestException e) {
     * setContent(buildLoginPanel(openInNewWindowDefault)); if (isAttached() && isVisible()) { center(); } }
     */
