...
// by GWT, special care is needed on the included functionality. Only the strictly needed functionality
// from the comprehensive library was included.
// See https://github.com/unbescape/unbescape/blob/master/src/main/java/org/unbescape/css/CssIdentifierEscapeUtil.java.
public class CssUtils {
  ...
  private static char[] toCompactHexa( final int codepoint, final char next, final int level ) {
  ...
  public static String escape( final String text  ) {
    final int level = 3;
    return escape( text, level );
  }

  /*
   * Perform an escape operation, based on String, according to the specified level and type.
   */
  private static String escape( final String text, final int level ) {
    ...
    for (int i = offset; i < max; i++) {
      ...
      if (codepoint < BACKSLASH_CHARS_LEN) {
