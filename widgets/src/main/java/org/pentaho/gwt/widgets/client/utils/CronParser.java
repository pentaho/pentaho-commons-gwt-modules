...
public class CronParser {
  ...
  public static final String DONT_CARE = "?"; //$NON-NLS-1$
  public static final String N_TH = "#"; //$NON-NLS-1$
  public static final String LAST = "L"; //$NON-NLS-1$
  private static final int MIN_CRON_FIELDS = 6;
  private static final int MAX_CRON_FIELDS = 7;

  enum CronField {
    SECONDS( 0 ), MINUTES( 1 ), HOURS( 2 ), DAY_OF_MONTH( 3 ), MONTH( 4 ), DAY_OF_WEEK( 5 ), YEAR( 6 );
  ...
  public enum RecurrenceType {
    Unknown, EveryWeekday, WeeklyOn, DayNOfMonth, NthDayNameOfMonth, LastDayNameOfMonth,
    EveryMonthNameN, NthDayNameOfMonthName, LastDayNameOfMonthName;
  ...
  private static final String MATCH_DAY_OF_MONTH_FIELD_RE =
      "^(\\*|\\?|L|([0-9]{1,2}(W|(,[0-9]{1,2}){0,30}))|([0-9]{1,2}[/-][0-9]{1,2}))$"; //$NON-NLS-1$

  public static boolean isDayOfMonthField( String fld ) {
    return fld.matches( MATCH_DAY_OF_MONTH_FIELD_RE );
  }

  private static final String MATCH_MONTH_FIELD_RE =
      "^(\\*|([0-9]{1,2}(,[0-9]{1,2}){0,11})|([0-9]{1,2}[/-][0-9]{1,2}))$"; //$NON-NLS-1$

  public static boolean isMonthField( String fld ) {
    return fld.matches( MATCH_MONTH_FIELD_RE );
  }

  private static final String MATCH_DAY_OF_WEEK_FIELD_RE =
      "^(\\*|\\?|L|([0-9]{1,2}(,[0-9]{1,2}){0,6})|([0-9]{1,2}[/#-][0-9]{1,2}))$"; //$NON-NLS-1$
  ...
  /**
   * NOTE: doesn't validate that month, day-of-month or day-of-week integers are in range. TODO sbarkdull, maybe it
   * should? Maybe return an int identifying the index of the character in the cron string where the first syntax error
   * is, if no error, return -1.
   * 
   * @param strInt
   * @return
   */
  ...
  /**
   * Abstract cron string: 0 MM HH ? * DAY#N (N ε {1,2,3,4} DAY ε {1,2,3,4,5,6,7})
   */
  private static final String NTH_DAY_NAME_OF_MONTH_RE =
      "^\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\?\\s+\\*\\s+[1-7]{1}#[1-4]{1}$"; //$NON-NLS-1$
  ...
  private static final String LAST_DAY_NAME_OF_MONTH_RE =
      "^\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\?\\s+\\*\\s+[1-7]{1}L$"; //$NON-NLS-1$

  /**
   * Abstract cron string: 0 MM HH ? * DAYL (DAY ε {1,2,3,4,5,6,7}, where these integers map to days of the week)
   * 
   * @param dayOfMonthToke
   * @param monthToke
   * @param dayOfWeekToke
  ...
  private static final String EVERY_MONTH_NAME_N_RE =
      "^\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\?$"; //$NON-NLS-1$
  ...
  private static final String N_DAY_NAME_OF_MONTH_NAME_RE =
      "^\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\?\\s+\\d{1,2}\\s+[1-7]{1}#[1-4]{1}$"; //$NON-NLS-1$
  ...
  private static final String LAST_DAY_NAME_OF_MONTH_NAME_RE =
      "^\\d{1,2}\\s+\\d{1,2}\\s+\\d{1,2}\\s+\\?\\s+\\d{1,2}\\s+\\d{1}L$"; //$NON-NLS-1$
   ...
   *           if the CRON string is an invalid CRON string
   */
  ...
  /**
   * Tokens: 1. The recurrence type, one of the values in the enum RecurrenceType 2. start second (0-59) 3. start minute
   * (0-59) 4. start hour (0-24) 5. if valid, a comma separated list of integers that map to the days of the week (1-7),
   * see isDaysOfWeekValid for valid RecurrenceTypes 6. if valid, an integer representing a day of the month (1-31), see
   * isDayOfMonthValid for valid RecurrenceTypes 7. if valid, an integer representing a day of the week (1-7), see
   * isWhichDayOfWeekValid for valid RecurrenceTypes 8. if valid, an integer representing a week of the month (1-4), see
   * isWhichWeekOfMonthValid for valid RecurrenceTypes 9. if valid, an integer representing a month of the year (1-12),
   * see isWhichMonthOfYearValid for valid RecurrenceTypes All or none of the items 5-9 may be present. Tokens are order
   * dependent. Tokens are not position dependent, meaning that the same token may occur and position N in one
   * recurrence type, and position N+1 in others.
   * 
   * @return
   * @throws CronParseException
   */
   ...
   *           if the recurrenceTokenString contains the Unknown RecurrenceType.
   */
  public static String recurrenceStringToCronString( String recurrenceTokenString ) throws RuntimeException,
    CronParseException {
    StringBuilder sb = new StringBuilder();
  ...
  public static void main( String[] args ) {
    ...
    String[] cronSamples = { "0 59 23 ? *", // invalid # tokens //$NON-NLS-1$
    ...
    try {
      ...
      System.out
          .println( "recurrenceTokens: " + recurrenceTokens + " [" + recurrenceStringToCronString( recurrenceTokens ) + "]  (" + cp.getCronString() + ")" ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    ...
    try {
      assert cp.getDayOfMonth() == 3 : "cp.getDayOfMonth() == 3"; //$NON-NLS-1$
    } catch ( CronParseException e1 ) {
      assert false : "CronParseException"; //$NON-NLS-1$
    }
    ...
    try {
      String recurrenceTokens = cp.getRecurrenceString();
      System.out.println( "recurrenceTokens: " + recurrenceTokens + " ["
          + recurrenceStringToCronString( recurrenceTokens ) + "]  (" + cp.getCronString() + ")" );
      assert cp.getCronString().equals( recurrenceStringToCronString( recurrenceTokens ) ) : "Failed on: " + recurrenceTokens; //$NON-NLS-1$
    } catch ( CronParseException e ) {
      e.printStackTrace();
    }
    ...
    try {
      ...
      assert false : "CronParseException";
    }
    ...
    cp = new CronParser( "0 1 2 28 2 ?" ); // EveryMonthNameN
    try {
      cp.parseToRecurrenceString();
    } catch ( CronParseException e ) {
      System.out.println( e );
    ...
    try {
      assert cp.getWhichDayOfWeek() == 5 : "cp.getWhichDayOfWeek() == 5"; //$NON-NLS-1$
    } catch ( CronParseException e1 ) {
      assert false : "CronParseException"; //$NON-NLS-1$
    }
