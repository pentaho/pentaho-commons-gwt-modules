 ...
 * first tries to execute the service and if the service returned an Response.SC_UNAUTHORIZED error, the user is
 * presented with a login dialog. Once the login is successful it performs the service which was request originally. If
 * the error is anything different than the unauthorized error, user is presented with the error dialog
 * 
 */
public class AuthenticatedGwtServiceUtil {
  ...
  /**
   * Display the login screen and and validate the credentials supplied by the user if the credentials are correct, the
   * execute method is being invoked other wise error dialog is being display. On clicking ok button on the dialog box,
   * login screen is displayed again and process is repeated until the user click cancel or user is successfully
   * authenticated
   * */
