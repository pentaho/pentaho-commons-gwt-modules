...
public class JSTextFormatterFactory {
  public static JavaScriptObject createFormatter(final String type, final String pattern) throws IllegalArgumentException {
    if ("number".equals(type) ||  //$NON-NLS-1$
      Number.class.getName().equals(type) ||
      ...
      "java.math.BigDecimal".equals(type) || //$NON-NLS-1$
      "java.math.BigInteger".equals(type)) //$NON-NLS-1$
    {
      return new JSNumberTextFormatter(pattern).getInstance();
    } else if ("date".equals(type) || //$NON-NLS-1$
      java.util.Date.class.getName().equals(type) ||    
